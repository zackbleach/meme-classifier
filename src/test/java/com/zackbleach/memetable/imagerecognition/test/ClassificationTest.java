...
public class ClassificationTest {
	...
	private static final Logger log = Logger.getLogger(ClassificationTest.class);
	public static final String TEST_FOLDER_PATH = "TestMemes/";
	
	/**
	 * Checks that two different {@link Meme}s have
	 * a distance greater than the distance threshold
	 * @throws IOException
	 */
	@Test
	public void calculateDistanceDifferent() throws IOException {
		String mallardDir = TEST_FOLDER_PATH+Meme.ADVICE_MALLARD.getDirectory();
	...
	/**
	 * Checks that two similar {@link Meme}s have
	 * a distance greater than the distance threshold
	 * @throws IOException
	 */
	@Test
	public void calculateDistanceSimilar() throws IOException {
		String mallardDir = TEST_FOLDER_PATH+Meme.ADVICE_MALLARD.getDirectory();
		String mallard2Dir = TEST_FOLDER_PATH+Meme.ADVICE_MALLARD.getDirectory();
	...
	/**
	 * For each {@link Meme} calls {@link getAccuracyForMeme()}
	 * TODO: Should this have a pass or fail condition?
	...
	/**
	 * Given a {@link Meme} will load all available test instances
	 * of that meme and attempt to classify them. 
	 * TODO: Get even amount of test data
	 * @param meme Meme to score accuracy of
	 * @return Amount of correct identifications / Amount of attempted identifications
	 * @throws IOException
	 */
	public float getAccuracyForMeme(Meme meme) throws IOException {
		float index = 0.0f;
	    float correct = 0.0f;
		List<BufferedImage> images = ImageUtils.readImagesFromFolder(TEST_FOLDER_PATH+meme.getDirectory());
		if (images.size() == 0) {
			return -1;
		}
	...
	/**
	 * Picks a random image from each directory provided
	 * and calculates the distance between them
	 * 
	 * TODO: When threshold is implemented use it for this test
	 * 
	 * @param directory1 Directory of first meme
	 * @param directory2 Directory of second meme
	 * @throws IOException
	 */
	public float calculateDistance(String directory1, String directory2) throws IOException {
